...
10. **全自动质量自测套件 (`tests/self_test.py`)**：
    - 自动化音频资产零静音/高能量校验；
    - 前端 JavaScript 语法与 134 处 DOM 挂载 100% 完整性校验；
