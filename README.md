...
## 🌐 云端平台 (GitHub Pages) 与 本地端工作站 (Local Station) 差异与分工说明

为了兼顾**「全球随时随地免安装快速体验」**与**「专业级工业级 AI 深度学习混音生产」**，系统采用了前瞻性的**分层双模 (Hybrid) 架构**。了解两者的分工有助于您在不同场景下获得最佳体验：

### 📊 功能特性详细对照矩阵

| 功能模块 | 线上云端平台 (GitHub Pages) | 本地端工作站 (`http://127.0.0.1:8000`) | 说明与技术原理 |
| :--- | :---: | :---: | :--- |
| **部署与运行环境** | 纯静态 CDN (免安装即开即用) | 本地 Python 3.11 + FastAPI 后端 | 云端由 GitHub 静态伺服；本地由 Mac 本地进程伺服 |
| **示范曲多轨播放与控制** | 🟢 **完全支持** | 🟢 **完全支持** | 抒情乡村风与 K-pop 流行示范曲音频文件内置于前端静态资源库 |
| **单轨推子/声相/Mute/Solo** | 🟢 **完全支持** | 🟢 **完全支持** | 浏览器原生 Web Audio 多轨走带与独立增益控制 |
| **Web Audio 实时参量混音** | 🟢 **完全支持** | 🟢 **完全支持** | 前端原生参数化 EQ、动态压缩器、高低通滤波与失真曲线 |
| **多版本风格实时热切** | 🟢 **完全支持** | 🟢 **完全支持** | 收音机、电话听筒、水下、8-Bit、空气感、俱乐部超重低音等实时 DSP 变奏 |
| **A/B 盲听与 8 频段频谱仪** | 🟢 **完全支持** | 🟢 **完全支持** | 快照 A/B/C 热切、动态 LED 增益衰减表 (GR) 与频谱仪动态绘制 |
| **ComfyUI 编曲提示词生成** | 🟢 **完全支持** | 🟢 **完全支持** | 根据调式与 BPM 自动生成 Stable Audio / DiffRhythm 结构化 Prompt |
| **Meta AI Demucs v4 音源分离** | ⚠️ **受限 (需本地算力)** | 🚀 **极速 2 秒纯净分离** | 深度神经网络需 PyTorch 与 Apple Silicon MPS GPU 硬件加速，静态云端无显卡 |
| **YouTube 链接解析与实时抓取** | ⚠️ **受限 (静态空间禁爬)** | 🚀 **全自动下载与无损转码** | 依赖后端 `yt-dlp` 与系统级 `FFmpeg` 音视频解封装转码 |
| **32-Bit 浮点母带与分轨导出** | 仅供前端试听录音 | 🚀 **广播级高保真 ZIP 打包** | 本地调用 Spotify Pedalboard 32-bit 浮点 DSP 进行无损脱机渲染与多轨打包 |

### 💡 为什么有这样的分工设计？
1. **彻底告别「假分离」带来的杂音困扰**：
   过去的纯前端简易工具仅使用粗糙的频段滤波来冒充音源分离，导致人声轨道严重混杂鼓点与伴奏残音。为了达到商业母带级纯净度，系统全面接入了业界首屈一指的 **Meta AI Demucs v4 (Hybrid Transformer)** 模型，该模型依赖深度学习框架与显卡算力，必须在具备运算硬件的环境下运行。
2. **场景化灵活使用**：
   - **对外展示与快速试听**：直接将 GitHub Pages 网址发送给团队、客户或在手机/平板上打开，即可快速体验多轨交互、试听不同混音版本的声学差异；
   - **专业生产与自备歌曲分轨**：在 Mac 上启动本地端服务，即可享受满血的 GPU 深度学习 4 轨分离、YouTube 自由抓取与 32-Bit 广播级母带导出。
